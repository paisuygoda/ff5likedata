...
(function() {
    ...
    Game_Battler.prototype.setDirectlyAttackAdditionalInfo = function(item) {
        ...
        var jump = getMetaValues(item, ['ジャンプ', 'Jump']);
        if (jump) info.sound = {"name":"Wind2","pan":0,"pitch":120,"volume":90};
        var land = getMetaValues(item, ['着地', 'Land']);
        if (land) info.sound = {"name":"Wind4","pan":0,"pitch":50,"volume":90};
        var slash = getMetaValues(item, ['Slash']);
        if (slash) info.sound = {"name":"Sword4","pan":0,"pitch":120,"volume":90};
    ...
    Game_Actor.prototype.requestCustomMotion = function(tagNames) {
        ...
        if (customMotion) {
            ...
            } else if (customMotion == "dance") {
                BattleManager.getSprite(this)._danceDuration = 16;
    ...
    Sprite_Battler.prototype.startAttackMotion = function(x, y, attackInfo, returnFlg) {
        ...
        if (!returnFlg) {
            if (additionalInfo.sound) AudioManager.playSe(additionalInfo.sound);
    ...
    Sprite_Battler.prototype.getDance = function() {
        if (!this._danceDuration) return;
        var variable = this._danceDuration;
        this._rotateX = - 32 * Math.cos(variable / 16 * Math.PI);
        this._rotateY = - 16 * Math.sin(variable / 16 * Math.PI);

        this._danceDuration--;
        if (!this._danceDuration) {
            this._rotateX = 0;
            this._rotateY = 0;
        }
    };
    ...
    // 【追加スクリプト】Sprite_Actorへの変更で競合してる部分を消す
    Sprite_AfterimageActor.prototype.updateBitmap = function() {
    ...
    Sprite_AfterimageActor.prototype.updateFrame = function() {
