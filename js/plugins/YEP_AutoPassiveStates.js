...
DataManager.processAPSNotetags2 = function(group) {
  ...
  // 魔法剣ステート
  var noteSwordElem = /<(?:SwordElemRate):[ ](\d+)>/i;
  var noteSwordState = /<(?:SwordState):[ ](\d+)>/i;
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      // 魔法剣ステート
      else if (line.match(noteSwordElem)) {
        obj.swordElemRate = parseInt(RegExp.$1);
      } else if (line.match(noteSwordState)) {
        obj.swordState = parseInt(RegExp.$1);
      }
    }
...
Game_Actor.prototype.passiveStatesRaw = function() {
    if (this._passiveStatesRaw) return this._passiveStatesRaw;
