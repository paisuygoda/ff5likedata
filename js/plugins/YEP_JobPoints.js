 ...
 * @default %1 gains %2%3!
 *
 * @param Ability Gain Text 1
 * @parent ---General---
 * @desc Adjusts how the ability gain text - line 1 is shown after battle.
 * %1 - Actor
 * @default %1はジョブレベルアップ
 *
 * @param Ability Gain Text 2
 * @parent ---General---
 * @desc Adjusts how the ability gain text - line 2 is shown after battle.
 * %1 - Ability
 * @default 「%1」 を手に入れた
 *
 * @param Alive Actors
...
// Yanfly.Param.AbilityTextFormat1 = String(Yanfly.Parameters['Ability Gain Text 1']);
// Yanfly.Param.AbilityTextFormat2 = String(Yanfly.Parameters['Ability Gain Text 2']);
Yanfly.Param.AbilityTextFormat1 = "%1はジョブレベルアップ！";
Yanfly.Param.AbilityTextFormat2 = "「%1」 を手に入れた";
...
DataManager.isDatabaseLoaded = function () {
  ...
  if (!Yanfly._loaded_YEP_JobPoints) {
    ...
    this.processJPNotetagsCursor($dataEnemies);
    this.processJPNotetagsLevel($dataEnemies);
    this.processJPNotetagsAbility($dataClasses);
    this.processJPNotetagsBonus($dataClasses);
    this.processJPNotetagsBonus($dataArmors);
...
// モンスターにレベルと倍率とメタモルアイテムを設定
DataManager.processJPNotetagsLevel = function (group) {
  var note1 = /<(?:Level):[ ](\d+)>/i;
  var note2 = /<(?:Rate):[ ](\d+)>/i;
  var note3 = /<(?:MRate):[ ](\d+)>/i;
  var note4 = /<(?:MetamolItem):[ ](\d+)[ ](\d+)>/i;
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    ...
    obj.rate = 1;
    obj.mrate = 1;
    obj.metamolItem = { category: 0, id: 0 };
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(note1)) {
        ...
        var itemId = parseInt(RegExp.$2);
        obj.metamolItem = { category: categoryId, id: itemId };
      }
    }
  }
};

DataManager.processJPNotetagsCursor = function (group) {
  var noteX = /<(?:CursorX):[ ](\d+)>/i;
  var noteY = /<(?:CursorY):[ ](\d+)>/i;
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    ...
    for (var i = 0; i < notedata.length; i++) {
      var line = notedata[i];
      if (line.match(noteX)) {
        obj.cursorX = parseFloat(RegExp.$1 * -0.01) + 0.5;
      } else if (line.match(noteY)) {
        obj.cursorY = parseFloat(RegExp.$1 * 0.01);
      }
    }
  }
};

DataManager.processJPNotetagsAbility = function (group) {
  var note1 = /<(?:Ability):[ ](\d+)[ ](\d+)>/i;
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    ...
    for (var i = 0; i < notedata.length; i++) {
      ...
      if (line.match(note1)) {
        var ability = {
          base_code: parseInt(RegExp.$1),
          point: parseInt(RegExp.$2),
        };
...
DataManager.processJPNotetagsBonus = function (group) {
  var note = /<(?:MasterBonus):[ ]([a-z]+)[ ](\d+)>/i;
  for (var n = 1; n < group.length; n++) {
    var obj = group[n];
    var notedata = obj.note.split(/[\r\n]+/);
    ...
    for (var i = 0; i < notedata.length; i++) {
      var line = notedata[i];
      if (line.match(note)) {
        if (String(RegExp.$1) === "mhp") {
          obj.masterbonus[0] = parseInt(RegExp.$2);
        } else if (String(RegExp.$1) === "mmp") {
          obj.masterbonus[1] = parseInt(RegExp.$2);
        }
      }
    }
  }
...
Game_Unit.prototype.fineMembers = function () {
  return this.members().filter(function (member) {
    return (
      member.isAlive() &&
      !member.isStateAffected(25) &&
      !member.isStateAffected(24)
    );
  });
};
...
BattleManager.gainJp = function () {
  ...
  members.forEach(function (actor) {
    ...
    var jobLevel = actor._jobLevel[actorClass.id];
    actor.gainJp(jp);
    while (actor._jobLevel[actorClass.id] > jobLevel) {
      var code =
        actor._actorId * 100 + actorClass.abilities[jobLevel]["base_code"];
      var item = $dataArmors[code];

      // 同じ名前のアビリティを持っていたら下位互換なので捨てる
      var abilityList = $gameParty.allItems().filter(function (ability) {
        return (
          ability.atypeId === actor._actorId + 6 &&
          ability.name.slice(0, ability.name.length - 3) ===
...
BattleManager.displayAbilities = function () {
  var abilities = this._rewards.abilities;
  if (abilities.length > 0) {
    $gameMessage.newPage();
    abilities.forEach(function (ability) {
      $gameMessage.add(
        "\\>" + Yanfly.Param.AbilityTextFormat1.format(ability.actorName)
      );
      $gameMessage.add(
        "\\>" + Yanfly.Param.AbilityTextFormat2.format(ability.abilityName)
      );
    });
  }
};
...
BattleManager.displayJpGain = function () {
  ...
  var text = fmt.format(jp, Yanfly.Param.Jp);
...
Game_Actor.prototype.initJobLevel = function () {
  this._jobLevel = [];
  for (var i = 0; i < $dataClasses.length; i++) {
    if (!this._jobLevel[i]) this._jobLevel.push(0);
  }
};
...
Game_Actor.prototype.gainJp = function (value, classId) {
  ...
  var addedPoint = this.jp(classId) + value;
  var curClass = this.currentClass();
  var nextPoint = curClass.abilities[this._jobLevel[curClass.id]]["point"];

  if (nextPoint === 0) {
    this.setJp(0, classId);
  } else if (addedPoint >= nextPoint) {
    while (nextPoint > 0 && addedPoint >= nextPoint) {
      this.setJp(addedPoint - nextPoint, classId);
      this._jobLevel[curClass.id] += 1;
      addedPoint -= nextPoint;
      nextPoint = curClass.abilities[this._jobLevel[curClass.id]]["point"];
    }
  } else {
    this.setJp(addedPoint, classId);
  }
...
Window_Base.prototype.drawActorClass = function (actor, wx, wy, ww) {
  ...
  this.drawActorJp(actor, classId, wx, wy + this.lineHeight(), ww, "right");
};
...
Window_Base.prototype.drawActorJp = function (actor, id, wx, wy, ww, align) {
  ...
  var curClass = actor.currentClass();
  ...
  var abp = fmt.format(numText, denomText);
  // すっぴんの場合はジョブレベル表示なし
  if (id === 22) {
    var text = "";
    // MASTER時
...
if (Imported.YEP_VictoryAftermath && Yanfly.Param.JpEnableAftermath) {
  ...
  Window_VictoryJp.prototype.drawJpGained = function (actor, rect) {
    ...
    var bonusJp =
      (1.0 * actor.battleJp() * this._tick) / Yanfly.Param.VAGaugeTicks;
