...
(function (exports) {
  ...
  exports.Param = (function () {
    ...
    ret.MenuCommandEnable = Boolean(
      parameters["Menu Command Enable"] === "true" || false
    );
    ret.CommandName = String(parameters["Command Name"]);
    ret.EffectAnimationId = Number(parameters["Effect Animation Id"]);
    ret.OptimizeEquipEnable = Boolean(
      parameters["Optimize Equip Enable"] === "true" || false
    );
  ...
  Utility.characterName = function (actor, data) {
    if (actor) {
      var characterName =
        "characterName" + ("0000" + actor.actorId()).slice(-4);
  ...
  Game_Actor.prototype.calcClassLevel = function (classId) {
    ...
    while (
      level < this.maxLevel() &&
      this.classExp(classId) >= this.expForLevelEx(classId, level + 1)
    ) {
      level++;
    }
    return level;
  ...
  Game_Actor.prototype.changeClassEx = function (classId, keepExp) {
    ...
    for (var i = 0; i < this.skillEquips().length; i++) {
      if (this.skillEquips()[i]) {
        this.changeSkillEquip(i, null);
      }
    }
    // 「たたかう」「アイテム」があったら消す
  ...
  exports.Sprite_ActorFace = (function () {
    ...
    Sprite_ActorFace.prototype.updatePosition = function () {
      ...
      var ph = Sprite_ActorFace._faceHeight;
      this.x = Math.floor(
        this._drawRect.x + Math.max(this._drawRect.width - pw, 0) / 2
      );
      this.y = Math.floor(
        this._drawRect.y + Math.max(this._drawRect.height - ph, 0) / 2
      );
    };
  ...
  exports.Window_JobChange = (function () {
    ...
    Window_JobChange.prototype.updateJobPosition = function () {
      ...
      for (var ii = 0; ii < num; ii++) {
        ...
        var offsetX = val % 7;
        var offsetY = Math.floor(val / 7);
        if (val % 7 === 0 || (val > 14 && val % 7 === 1)) {
          offsetX += 7;
          offsetY -= 1;
        }
        offsetX -= offsetY / 2;
        sprite.setOffset(
          offsetX * sprite.width * 1.5,
          offsetY * sprite.height * 1.6
        );

        // Show JobLevel
        var jobLevel = this._actor._jobLevel[classId];
        var jobLevelText;
        if (jobLevel === 4) {
          this.drawTextEx(
            "\\i[90]",
            -17 + offsetX * sprite.width * 1.5,
            offsetY * sprite.height * 1.6 - 5,
            50,
            "center"
          );
        } else if (classId === 22) {
          var allMaster = true;
          for (var iii = 1; iii < 22; iii++) {
            if (this._actor._jobLevel[iii] < 4) allMaster = false;
          }
          if (allMaster) {
            this.drawTextEx(
              "\\i[90]",
          ...
          jobLevelText = "Lv." + String(jobLevel);
          this.contents.fontSize = 18;
          this.drawText(
            jobLevelText,
            -22 + offsetX * sprite.width * 1.5,
            offsetY * sprite.height * 1.6 - 5,
            50,
            "center"
          );
    ...
    Window_JobChange.prototype.drawIcon = function (iconIndex, x, y) {
    ...
    Window_JobChange.prototype.isShowChangeableClass = function (
      actor,
      classId
    ) {
      var result = false;
      ...
      if (data.meta.displayStats) {
        if (data.meta.displayStats === "hide") {
          // 条件を満たしていない場合は非表示
          if (!actor.meetsRequirements(classId)) {
            return false;
          }
        } else if (data.meta.displayStats === "both") {
          // 誰一人条件を満たしていない場合は非表示
    ...
    // カーソル移動でステータス変化させる
    const WiJo_processCursorMove = Window_JobChange.prototype.processCursorMove;
    Window_JobChange.prototype.processCursorMove = function () {
    ...
    Window_JobChange.prototype.itemRect = function (index) {
      ...
      if (this._jobSprite.length > 0) {
        ...
        var sprite = this._jobSprite[classId];
        rect.x =
          sprite.x - sprite.width * sprite.anchor.x - this.standardPadding();
        rect.y =
          sprite.y - sprite.height * sprite.anchor.y - this.standardPadding();
    ...
    Window_JobChange.prototype.setCursorRect = function (
      x,
      y,
      width,
      height
  ...
  exports.Scene_JobChange = (function () {
    ...
    Scene_JobChange.prototype.createJobChangeWindow = function () {
      ...
      this._jobWindow._statusWindow = this._statusWindow;
      this._jobWindow.activate();
      this.addWindow(this._jobWindow);
    };

    Scene_JobChange.prototype.popScene = function () {
      SceneManager.pop();
    };
    ...
    Scene_JobChange.prototype.update = function () {
      if (this.exitState === 2) SceneManager.goto(Scene_SkillEquip);
      if (this.exitState && this._dialogWindow.isClosed()) this.exitState = 2;
      if (this._dialogWindow.isOpen()) this.exitState = 1;
