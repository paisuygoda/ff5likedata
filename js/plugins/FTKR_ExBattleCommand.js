 ...
 * 
 * !!!オリジナルより改変!!!
 * priorityの整数部が等しいスキルは同質のスキルであり、
 * 小数点以下が高いほど上位互換であるという前提を置く
 * 小数部が0.5未満に収まるものとして、下位互換スキルを削除する機能を追加
 * 【追加スクリプト】のタグをコメントアウトに残しておくので参照
 * 
 *-----------------------------------------------------------------------------
 * 設定方法
 *-----------------------------------------------------------------------------
...
(function() {
    ...
    Game_Actor.prototype.sortBattleCommands = function() {
        ...
        var rawCommands = this.battleCommands().sort(function(a, b) {
            var p1 = a.priority;
            var p2 = b.priority;
            if (p1 !== p2) {
                return p2 - p1;
	...
	// 予言をMP<7の時に使えないようにするためにevalでスクリプトを書くが、<>がnoteコマンドで認識されてしまうのでそのためのfunction
	Window_Command.prototype.castable = function(mp, cost) {
        return mp > cost;
    };
    ...
    Window_ActorCommand.prototype.isCommandEnabled = function(index) {
        return this.isEbcCommandEnabled(index)
                && this._actor.skillFrogCheck(this.commandName(index));
    };

    //書き換え
    Window_ActorCommand.prototype.isCurrentItemEnabled = function() {
        return this.isEbcCurrentItemEnabled()
                && this._actor.skillFrogCheck(this.commandName(this.index()));
    };

    Game_BattlerBase.prototype.skillFrogCheck = function(name) {
        return (name === '攻撃' || name === 'アイテム' || name === '防御' || name === '黒魔法') ? true : !this.isStateAffected(12);
    }
    ...
    Window_ActorCommand.prototype.addCustomCommand = function(cmd) {
        // 隠れているときのコマンド変更 DONE: かくれる・あらわれるスキルIDのハードコーディング
        if (cmd.skillId === 34 && this._actor.isStateAffected(32)) cmd = {
            code     : cmd.code,
            dataId   : cmd.dataId,
            ext      : cmd.ext,
            enabled  : cmd.enabled,
