 ...
 *
 * Input the character name with \N format
...
(function () {
  ...
  class Window_ActorName extends Window_Base {
    ...
    // udonGeneration: 改変ここから
    setText(textColor, text) {
      this._text = text;
      this._textColor = textColor;
      // udonGeneration: 改変ここまで
      this.refresh();
    }
    refresh() {
      ...
      this.changeTextColor(
        this.textColor(this._textColor ? this._textColor : params.textColor)
      );
      // udonGeneration: 改変ここまで
      this.drawText(
        this._text,
    ...
    processActorName(text) {
      ...
      //     return '';
      // });
      return text.replace(/\x1bN\<([0-9]*)(.*?)\>/gi, (whole, ...args) => {
        this.setText(args[0], args[1]);
        this.show();
        this.open();
        this.activate();
        return "";
  ...
  const Window_Message_createSubWindows =
  ...
  const Window_Message_terminateMessage =
  ...
  const Window_Message_updatePlacement =
  ...
  const Window_Message_updateBackground =
